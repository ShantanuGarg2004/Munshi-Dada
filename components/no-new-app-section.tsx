        ...
        <div className="grid lg:grid-cols-2 gap-10 sm:gap-16 items-center mb-12 sm:mb-20">
          <div>
            ...
            <div className="no-app-cta flex flex-wrap gap-3">
              <Button className="bg-[#25D366] hover:bg-[#1fba5a] text-white rounded-full px-6 sm:px-7 py-5 sm:py-6 text-sm sm:text-[15px] font-bold shadow-lg shadow-[#25D366]/20 transition-all duration-300 hover:scale-105">
                Get Access
              </Button>
              <Button variant="outline" className="rounded-full px-6 sm:px-7 py-5 sm:py-6 text-sm sm:text-[15px] font-semibold border-white/40 text-white bg-white/10 hover:bg-white/20 hover:border-white/60 transition-all duration-300 group">
                Contact Us
          ...
          <div className="flex flex-col items-center gap-4">
            <div className="bg-white/5 border border-white/10 rounded-2xl sm:rounded-3xl p-5 sm:p-8 w-full">
              <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-5 sm:mb-6 text-center">How it flows</p>
              <div className="flex items-center justify-between gap-1 sm:gap-2 overflow-x-auto">
                {[
                  ...
                  "arrow" in item ? (
                    <span key={i} className="text-[#25D366] text-base sm:text-xl font-bold flex-shrink-0">→</span>
                  ) : "wa" in item ? (
                    <div key={i} className="bg-[#25D366] rounded-xl sm:rounded-2xl px-3 sm:px-4 py-2 sm:py-3 text-center shadow-lg shadow-[#25D366]/20 flex-shrink-0">
                      <svg viewBox="0 0 24 24" className="w-5 h-5 sm:w-6 sm:h-6 mx-auto mb-1" fill="white">
                      ...
                      <span className="text-[10px] sm:text-xs font-bold text-white">WA</span>
                    </div>
                  ) : (
                    <div key={i} className="bg-white/6 border border-white/10 rounded-xl sm:rounded-2xl px-2 sm:px-4 py-2 sm:py-3 text-center flex-shrink-0">
                      <div className="text-xl sm:text-2xl mb-1">{item.emoji}</div>
                      <span className="text-[9px] sm:text-xs font-medium text-white/60">{item.label}</span>
                    </div>
                  )
                )}
              </div>
              <p className="text-xs text-gray-500 text-center mt-4 sm:mt-5 leading-relaxed">Your whole operation runs through one familiar app.</p>
            </div>
          </div>
        </div>
