...
export function FeaturesSection() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.fromTo(".features-header", { opacity: 0, y: 50 }, { opacity: 1, y: 0, duration: 0.8, ease: "power3.out", scrollTrigger: { trigger: ".features-header", start: "top 80%" } })

      const featureBlocks = gsap.utils.toArray<HTMLElement>(".feature-block")
      featureBlocks.forEach((block, index) => {
        const isEven = index % 2 === 0
        gsap.fromTo(block.querySelector(".feature-content"), { opacity: 0, x: isEven ? -80 : 80 }, { opacity: 1, x: 0, duration: 0.9, ease: "power3.out", scrollTrigger: { trigger: block, start: "top 75%" } })
        const visual = block.querySelector(".feature-visual")
        if (visual) {
          gsap.fromTo(visual, { opacity: 0, x: isEven ? 80 : -80, scale: 0.92 }, { opacity: 1, x: 0, scale: 1, duration: 0.9, ease: "power3.out", delay: 0.15, scrollTrigger: { trigger: block, start: "top 75%" } })
        }
        gsap.fromTo(block.querySelectorAll(".feature-check"), { opacity: 0, x: -20 }, { opacity: 1, x: 0, duration: 0.4, stagger: 0.1, ease: "power2.out", scrollTrigger: { trigger: block, start: "top 70%" } })
      })
    }, sectionRef)
    ...
    <section ref={sectionRef} className="py-24 bg-white overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="features-header text-center mb-20">
          <p className="text-xs font-semibold text-[#25D366] uppercase tracking-widest mb-3">HOW MUNSHEE MAKES YOUR LIFE EASY</p>
        ...
        <div className="space-y-28">
          {features.map((feature, index) => {
            const isEven = index % 2 === 0
            return (
              <div key={index} className={`feature-block grid lg:grid-cols-2 gap-14 items-center`}>
                {/* Visual Side */}
                <div className={`feature-visual ${isEven ? "order-1 lg:order-2" : "order-1"}`}>
                  {feature.visual === "cards" ? (
                    <div className={`${feature.cardBg} rounded-3xl p-10`}>
                        ...
                        {feature.cardItems?.map((item, i) => (
                          <div key={i} className="bg-white rounded-xl p-4 shadow-sm border border-white hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-default">
                            ...
                            <div className="mt-2 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                              <div className="h-full bg-[#25D366] rounded-full" style={{ width: `${60 + i * 10}%` }}></div>
                            </div>
                          </div>
                        ))}
                      </div>
                      <div className="mt-4 bg-white rounded-xl p-4 shadow-sm border border-white">
                        <div className="flex items-center gap-3">
                          ...
                          <p className="text-xs text-gray-500">Munshee auto-collected all updates ✅</p>
                        </div>
                      </div>
                    </div>
                  ) : (
                ...
                <div className={`feature-content ${isEven ? "order-2 lg:order-1" : "order-2"}`}>
                  <span className="inline-block text-xs font-semibold text-[#25D366] uppercase tracking-widest bg-[#25D366]/10 rounded-full px-3 py-1 mb-5">{feature.label}</span>
